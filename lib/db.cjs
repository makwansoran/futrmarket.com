...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    if (updates.trending !== undefined) {
      ...
      // We'll try to update with trending, but if it fails, we'll handle it
    }
    ...
    if (error) {
      ...
      console.error("Updates:", JSON.stringify(updatesToApply, null, 2));
    ...
    if (!data) {
      console.error("No contract found with ID:", contractId);
