...
async function createContract(contractData) {
  if (isSupabaseEnabled()) {
    ...
    // Check ALL contracts, not just recent ones, to prevent any duplicates
    ...
    if (question) {
      ...
      if (!exactError && exactMatches && exactMatches.length > 0) {
        ...
        console.warn("[DB] Duplicate contract detected (exact match):", {
        ...
        if (!fetchError && existingContract) {
          console.log("[DB] Returning existing contract instead of creating duplicate:", existingContract.id);
          return existingContract;
        }
      }
      
      // Also check for case-insensitive matches (normalized comparison)
      ...
      if (!allError && allContracts) {
        ...
        const caseInsensitiveMatch = allContracts.find(c => 
          String(c.question || "").trim().toLowerCase() === normalizedQuestion
        );
        
        if (caseInsensitiveMatch) {
          console.warn("[DB] Duplicate contract detected (case-insensitive match):", {
            existingId: caseInsensitiveMatch.id,
            existingCreatedAt: caseInsensitiveMatch.created_at,
            existingQuestion: caseInsensitiveMatch.question,
            newQuestion: question
          });
          // Return the existing contract instead of creating a duplicate
          const { data: existingContract, error: fetchError } = await supabase
    ...
    if (error) {
      ...
      // Handle duplicate primary key error (shouldn't happen with our check, but just in case)
  ...
  // For file-based storage, check for duplicates
  const contracts = loadJSON(path.join(DATA, "contracts.json"));
  const question = String(contractData.question || "").trim();
  if (question) {
    const fiveMinutesAgo = Date.now() - (5 * 60 * 1000);
    const existingContract = Object.values(contracts).find(c => 
      String(c.question || "").trim() === question && 
      (c.created_at || c.createdAt || 0) >= fiveMinutesAgo
    );
    if (existingContract) {
      console.log("[DB] Duplicate contract detected in file storage, returning existing:", existingContract.id);
      return existingContract;
    }
  }
...
async function removeDuplicateContracts() {
  if (isSupabaseEnabled()) {
    ...
    // Group contracts by question (case-insensitive, trimmed)
