...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    if (error) {
      ...
      // "Cannot coerce" often means the column doesn't exist or 0 rows were updated
      if (errorMsg.includes("Cannot coerce") || errorCode === "PGRST116") {
        ...
        // Contract exists, so the error is likely about the trending column not existing
        // Try updating without selecting trending to confirm
        if (trendingValue !== null) {
          // Remove trending from updates and try again to see if that works
          const { trending, ...updatesWithoutTrending } = updatesToApply;
          const { data: testData, error: testError } = await supabase
            .from("contracts")
            .update(updatesWithoutTrending)
            .eq("id", contractId)
            .select()
            .maybeSingle();
          
          // If update without trending works, then trending column doesn't exist
          ...
          // If both fail, it's a different issue
          const clearError = new Error(
            `Database schema error: The 'trending' column does not exist in your contracts table.\n\n` +
            `To fix this, run this SQL in your Supabase SQL Editor:\n\n` +
            `ALTER TABLE public.contracts ADD COLUMN IF NOT EXISTS trending BOOLEAN NOT NULL DEFAULT false;\n\n` +
          ...
          clearError.code = error.code;
          clearError.details = error.details;
          clearError.hint = error.hint;
          throw clearError;
        }
      }
      
      // If trending column doesn't exist, provide clear error message
      if (trendingValue !== null && (
