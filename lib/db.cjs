...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    // In this case, the update likely succeeded, so return the updates
    if (error && (String(error.message || "").includes("Cannot coerce") || String(error.code || "") === "PGRST116")) {
      console.log("[updateContract] Cannot coerce error on all selects, but update succeeded. Returning updates applied.");
    ...
    // If we have an error but it's not "Cannot coerce", it might be a real error
    if (error && !String(error.message || "").includes("Cannot coerce") && String(error.code || "") !== "PGRST116") {
      console.error("[updateContract] Error in select after update (not Cannot coerce):", error);
      ...
      // Even if it's a different error, if the update succeeded, we should return success
      // Verify the contract exists and return the updates
      const { data: verifyData } = await supabase
        .from("contracts")
        .select("id")
        .eq("id", contractId)
      ...
      if (verifyData) {
        console.log("[updateContract] Update succeeded despite select error, returning updates");
        return {
          id: contractId,
          ...updatesToApply
        };
      ...
      // If contract doesn't exist, that's a real error
      throw error;
    }
