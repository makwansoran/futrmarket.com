...
async function createUser(userData) {
  ...
  // Generate a deterministic placeholder wallet address from email for email-based users
  const crypto = require("crypto");
  const emailHash = crypto.createHash("sha256").update(email).digest("hex");
  const placeholderWallet = "0x" + emailHash.substring(0, 40).toLowerCase(); // 40 hex chars = 20 bytes, lowercase
  
  const user = {
    wallet_address: placeholderWallet, // PRIMARY KEY - required!
    email,
  ...
  try {
    const { error: balanceError } = await supabase
      .from("balances")
      .insert({
        wallet_address: placeholderWallet, // Use wallet_address, not email
        cash: 0,
        portfolio: 0,
        on_chain_balance: 0,
        updated_at: Date.now()
    ...
    if (balanceError) {
      ...
      console.log("✅ [createUser] Balance created successfully for user:", placeholderWallet);
    }
  } catch (balanceError) {
