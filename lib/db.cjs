...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    if (error && (String(error.message || "").includes("Cannot coerce") || String(error.code || "") === "PGRST116")) {
      ...
      // Contract exists, so the update likely worked. Try to get it without problematic columns
      const { data: simpleData, error: simpleError } = await supabase
        .from("contracts")
        .select("id, question, category, trending, live, featured, status, resolution, image_url, created_at")
        .eq("id", contractId)
        .maybeSingle();
      ...
      // If that also fails, the update worked but we can't select - this is unusual
      console.warn("[updateContract] Update succeeded but cannot select contract. This may indicate a schema issue.");
    }
