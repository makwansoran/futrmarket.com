...
async function getUser(email) {
  if (isSupabaseEnabled()) {
    ...
    // Use wallet-first database function for consistency
    const { getUserByEmail } = require("./db-wallet-first.cjs");
...
async function createUser(userData) {
  ...
  // ONLY use Supabase - no file-based fallback
  if (!isSupabaseEnabled()) {
    const error = new Error("Supabase is not enabled. Cannot create user without database connection.");
    console.error("❌ CRITICAL ERROR:", error.message);
    console.error("   Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env file");
    throw error;
  }
  
  // Database uses wallet-first schema - wallet_address is PRIMARY KEY (NOT NULL)
  // Create a real wallet using thirdweb API, or fallback to deterministic wallet
  const { createWalletForUser } = require("./thirdweb-wallet.cjs");
  let walletAddress;
  
  try {
    const walletResult = await createWalletForUser(email);
    walletAddress = walletResult.walletAddress;
    console.log("✅ [createUser] Wallet created/retrieved for user:", walletAddress);
  } catch (walletError) {
    console.error("❌ [createUser] Error creating wallet, using deterministic fallback:", walletError.message);
    // Fallback to deterministic wallet if thirdweb fails
    const crypto = require("crypto");
    const emailHash = crypto.createHash("sha256").update(email).digest("hex");
    walletAddress = "0x" + emailHash.substring(0, 40).toLowerCase();
  }
  
  const user = {
    wallet_address: walletAddress, // PRIMARY KEY - required!
    email,
  ...
  console.log("🔵 [createUser] Attempting to create user in Supabase (wallet-first schema):", { 
  ...
  if (error) {
    ...
    console.error("❌ [createUser] User data attempted:", JSON.stringify(user, null, 2));
    throw error;
  }
  
  console.log("✅ [createUser] User created successfully in Supabase:", { 
    wallet_address: walletAddress, 
    email 
  });
  
  // Also create balance entry for the user (using wallet_address as primary key)
  // IMPORTANT: Use the wallet_address from the created user (data.wallet_address) to ensure consistency
  const userWalletAddress = data?.wallet_address || walletAddress;
  
  try {
    // Verify user exists before creating balance (wait a tiny bit for transaction to commit)
    // Use upsert instead of insert to handle race conditions
    const { error: balanceError } = await supabase
      .from("balances")
      .upsert({
        wallet_address: userWalletAddress, // Use the actual wallet_address from created user
        cash: 0,
        portfolio: 0,
        on_chain_balance: 0,
        updated_at: Date.now()
      ...
      if (balanceError) {
        console.error("⚠️  [createUser] Error creating balance:", balanceError);
        console.error("⚠️  [createUser] Balance error details:", JSON.stringify(balanceError, null, 2));
        // If it's a foreign key error, the user might not be committed yet - retry once
        if (balanceError.code === '23503' || balanceError.message?.includes('foreign key')) {
          console.log("🔄 [createUser] Retrying balance creation after short delay...");
          ...
          if (retryError) {
            console.error("❌ [createUser] Balance creation failed on retry:", retryError);
          } else {
            console.log("✅ [createUser] Balance created successfully on retry:", userWalletAddress);
          }
        }
      } else {
        console.log("✅ [createUser] Balance created successfully for user:", userWalletAddress);
      }
  } catch (balanceError) {
    console.error("⚠️  [createUser] Exception creating balance:", balanceError);
    // Don't throw - balance creation failure shouldn't prevent user creation
  }
...
// BALANCES
// ============================================
// Helper function to get wallet_address from email (for wallet-first schema)
function getWalletAddressFromEmail(email) {
  const crypto = require("crypto");
  const emailHash = crypto.createHash("sha256").update(email.toLowerCase()).digest("hex");
  return "0x" + emailHash.substring(0, 40).toLowerCase();
}
...
async function getBalance(email) {
  ...
  if (isSupabaseEnabled()) {
    // Wallet-first schema: balances table uses wallet_address as PRIMARY KEY
    // IMPORTANT: Get the actual wallet_address from the user record, not generate it
    // This ensures we use the same wallet_address that was used to create the user
    let walletAddress;
    try {
      ...
      if (user && user.wallet_address) {
        ...
        // Fallback: if user doesn't exist, use deterministic generation
        walletAddress = getWalletAddressFromEmail(emailLower);
      }
    } catch (userError) {
      // Fallback: if error fetching user, use deterministic generation
      walletAddress = getWalletAddressFromEmail(emailLower);
    }
  ...
  if (isSupabaseEnabled()) {
    ...
    try {
      ...
      if (user && user.wallet_address) {
        ...
        // Fallback: if user doesn't exist or has no wallet_address, use deterministic generation
        console.warn("⚠️  [updateBalance] User not found or has no wallet_address, using deterministic fallback");
        walletAddress = getWalletAddressFromEmail(emailLower);
      }
    } catch (userError) {
      console.error("⚠️  [updateBalance] Error fetching user, using deterministic fallback:", userError);
      walletAddress = getWalletAddressFromEmail(emailLower);
    }
    ...
    const { error } = await supabase
      .from("balances")
      .upsert({
        wallet_address: walletAddress, // Use the actual wallet_address from user record
        cash: updates.cash !== undefined ? updates.cash : 0,
        portfolio: updates.portfolio !== undefined ? updates.portfolio : 0,
        on_chain_balance: updates.on_chain_balance !== undefined ? updates.on_chain_balance : 0,
        updated_at: Date.now()
      }, {
        onConflict: "wallet_address"
      });
    ...
    if (error) {
      ...
      // If it's a foreign key error, the user might not exist
      if (error.code === '23503' || error.message?.includes('foreign key')) {
        throw new Error(`Cannot update balance: User with wallet_address ${walletAddress} does not exist in users table. Please create the user first.`);
      }
      throw error;
    }
...
async function createContract(contractData) {
  if (isSupabaseEnabled()) {
    ...
    // Country is required, so always include it (even if null, but it shouldn't be null)
    // Make sure country is included in the insert
    if (insertData.country === undefined) {
      console.warn("[DB] Country field is undefined in contractData, this should not happen");
    }
    
    console.log("[DB] Creating contract in Supabase with country:", insertData.country);
    console.log("[DB] Full contract data:", JSON.stringify(insertData, null, 2));
    
    // Ensure country is included (it's required)
    if (!insertData.country || insertData.country.trim() === "") {
      console.error("[DB] ⚠️ WARNING: Country is missing or empty in contract data!");
      console.error("[DB] Contract data:", JSON.stringify(insertData, null, 2));
    }
    ...
    if (data && data.country) {
      console.log("[DB] ✅ Contract created successfully with country:", data.country);
    } else if (data) {
      console.warn("[DB] ⚠️ Contract created but country field is missing in response:", data);
    }
...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    console.log("[updateContract] Updates to apply:", JSON.stringify(updatesToApply, null, 2));
    if (updatesToApply.country !== undefined) {
      console.log("[updateContract] Country update:", updatesToApply.country);
    }
    ...
    const selectStrategies = [
      ...
      // Strategy 2: Select specific columns including trending and country
      () => supabase.from("contracts").select("id, question, description, category, trending, live, featured, status, resolution, image_url, expiration_date, competition_id, subject_id, country, market_price, buy_volume, sell_volume, total_contracts, volume, yes_price, no_price, yes_shares, no_shares, created_at, created_by").eq("id", contractId).maybeSingle(),
...
async function getPosition(email, contractId) {
  ...
  if (isSupabaseEnabled()) {
    // Wallet-first schema: positions table uses wallet_address, not email
    const walletAddress = getWalletAddressFromEmail(emailLower);
...
async function upsertPosition(email, contractId, positionData) {
  ...
  if (isSupabaseEnabled()) {
    ...
    const { data, error } = await supabase
      .from("positions")
      .upsert({
        wallet_address: walletAddress, // Use wallet_address, not email
        contract_id: contractId,
        ...positionData,
        updated_at: Date.now()
      }, {
        onConflict: "wallet_address,contract_id" // Update conflict resolution
      })
      .select()
      .single();
...
async function createOrder(orderData) {
  if (isSupabaseEnabled()) {
    // Wallet-first schema: orders table uses wallet_address, not email
    // If orderData has email, convert it to wallet_address
    const orderInsert = { ...orderData };
    if (orderInsert.email && !orderInsert.wallet_address) {
      orderInsert.wallet_address = getWalletAddressFromEmail(orderInsert.email);
      delete orderInsert.email; // Remove email field
    }
...
// Get user email by wallet address (reverse lookup)
async function getUserByWalletAddress(walletAddress) {
  ...
  // File-based lookup
  const wallets = loadJSON(path.join(DATA, "wallets.json"));
  for (const [email, wallet] of Object.entries(wallets)) {
    if (wallet && wallet.evm_address && wallet.evm_address.toLowerCase() === addressLower) {
      return email;
    }
  }
  return null;
...
async function getVerificationCode(email) {
  ...
  if (isSupabaseEnabled()) {
    // Wallet-first schema: verification_codes uses wallet_address as PRIMARY KEY
    // IMPORTANT: Get the actual wallet_address from the user record, not generate it
    let walletAddress;
    try {
      const user = await getUser(emailLower);
  ...
  if (isSupabaseEnabled()) {
    ...
    try {
      ...
      if (user && user.wallet_address) {
        ...
        // Fallback: if user doesn't exist, use deterministic generation
        // But this should rarely happen since we create user before verification code
        console.warn("⚠️  [upsertVerificationCode] User not found, using deterministic wallet address");
        walletAddress = getWalletAddressFromEmail(emailLower);
      }
    } catch (userError) {
      console.error("⚠️  [upsertVerificationCode] Error fetching user, using deterministic fallback:", userError);
      walletAddress = getWalletAddressFromEmail(emailLower);
    }
    ...
    if (error) {
      ...
      console.error("Error details:", JSON.stringify(error, null, 2));
      // If it's a foreign key error, the user might not exist yet
      if (error.code === '23503' || error.message?.includes('foreign key')) {
        throw new Error(`Cannot create verification code: User with wallet_address ${walletAddress} does not exist. Please create the user first.`);
      }
      throw error;
    }
    return data;
...
async function getDeposits(email) {
  ...
  if (isSupabaseEnabled()) {
    // Wallet-first schema: deposits table uses wallet_address, not email
    const walletAddress = getWalletAddressFromEmail(emailLower);
...
async function getTransactions(email) {
  ...
  if (isSupabaseEnabled()) {
    // Wallet-first schema: transactions table uses wallet_address, not email
    const walletAddress = getWalletAddressFromEmail(emailLower);
...
async function getAllFeatures(filterActive = false) {
  if (isSupabaseEnabled()) {
    console.log(`🔵 [getAllFeatures] Starting query - filterActive: ${filterActive}`);
    
    // First, try to get ALL features (no filter) to see what's in the database
    ...
    if (allError) {
      console.error("❌ [getAllFeatures] Error fetching ALL features:", allError);
      console.error("❌ [getAllFeatures] Error code:", allError.code);
      console.error("❌ [getAllFeatures] Error message:", allError.message);
      // Check if it's an RLS error
      if (allError.code === '42501' || allError.message?.includes('row-level security')) {
        console.error("❌ [getAllFeatures] RLS is blocking access to features table!");
        console.error("   Run the SQL script: supabase/disable-rls-simple.sql");
        console.error("   Or add RLS policies for service_role on features table");
      }
      return [];
    }
    
    console.log(`🔵 [getAllFeatures] Found ${allData?.length || 0} total features in database`);
    if (allData && allData.length > 0) {
      console.log(`🔵 [getAllFeatures] All features:`, 
        allData.map(f => ({ id: f.id, title: f.title, status: f.status })));
    }
    
    // Now apply the filter if needed
    let features = allData || [];
    ...
    if (filterActive) {
      ...
      features = features.filter(f => {
        ...
        if (!isActive) {
          console.log(`🔵 [getAllFeatures] Filtering out feature: ${f.id} (status: "${status}")`);
        }
        return isActive;
      });
      
      if (beforeCount !== features.length) {
        console.warn(`[getAllFeatures] Filtered out ${beforeCount - features.length} features with non-matching status`);
        const filteredOut = allData.filter(f => {
          const status = String(f.status || "").trim();
          return status.toLowerCase() !== "active";
        });
        console.warn(`[getAllFeatures] Features with other statuses:`, 
          filteredOut.map(f => ({ id: f.id, title: f.title, status: f.status })));
      }
    }
    
    console.log(`[getAllFeatures] filterActive=${filterActive}, returning ${features.length} features`);
    if (filterActive && features.length > 0) {
      console.log("[getAllFeatures] ✅ Active features:", features.map(f => ({ id: f.id, title: f.title, status: f.status })));
    } else if (filterActive && features.length === 0) {
      console.warn("[getAllFeatures] ⚠️ No active features found!");
      if (allData && allData.length > 0) {
        console.warn(`   Found ${allData.length} total features, but none with status='Active'`);
        console.warn("   Available statuses:", [...new Set(allData.map(f => f.status))]);
      } else {
        console.warn("   No features found in database at all!");
        console.warn("   Check: 1. Features exist in Supabase 2. RLS is disabled 3. Service role key is working");
      }
    }
  ...
  // Filter by active status if requested (case-insensitive)
  if (filterActive) {
    return featuresArray.filter(f => {
      const status = String(f.status || "").trim();
      return status.toLowerCase() === "active";
    });
  }
...
async function createFeature(featureData) {
  if (isSupabaseEnabled()) {
    ...
    console.log("🔵 [createFeature] Attempting to create feature:", { 
      id: insertData.id, 
    ...
    if (error) {
      console.error("❌ [createFeature] Error creating feature:", error);
      console.error("❌ [createFeature] Error code:", error.code);
      console.error("❌ [createFeature] Error message:", error.message);
      
      // Check if it's an RLS error
      if (error.code === '42501' || error.message?.includes('row-level security')) {
        console.error("❌ [createFeature] RLS is blocking feature creation!");
        console.error("   Run the SQL script: supabase/disable-rls-simple.sql");
        console.error("   Or add RLS policies for service_role on features table");
      }
      ...
      if (isSubjectIdError && hasSubjectId) {
        ...
        if (retryError) {
          console.error("❌ [createFeature] Error creating feature (retry):", retryError);
          throw retryError;
        }
        console.log("✅ [createFeature] Feature created successfully (without subject_id)");
        return retryData;
      }
    ...
    console.log("✅ [createFeature] Feature created successfully:", { 
      id: data?.id, 
      title: data?.title, 
      status: data?.status 
    });
...
async function updateFeature(featureId, updates) {
  if (isSupabaseEnabled()) {
    ...
    if (updates.type !== undefined) dbUpdates.type = String(updates.type).trim();
    if (updates.status !== undefined) dbUpdates.status = String(updates.status).trim();
    if (updates.imageUrl !== undefined) dbUpdates.image_url = updates.imageUrl ? String(updates.imageUrl).trim() : null;
    if (updates.url !== undefined) dbUpdates.url = updates.url ? String(updates.url).trim() : null;
    if (updates.subjectId !== undefined) {
      if (updates.subjectId && String(updates.subjectId).trim()) {
        dbUpdates.subject_id = String(updates.subjectId).trim();
      } else {
        dbUpdates.subject_id = null;
      }
    }
  ...
  // Update fields
  if (updates.title !== undefined) features[featureId].title = String(updates.title).trim();
  if (updates.description !== undefined) features[featureId].description = String(updates.description).trim();
  if (updates.type !== undefined) features[featureId].type = String(updates.type).trim();
  if (updates.status !== undefined) features[featureId].status = String(updates.status).trim();
  if (updates.imageUrl !== undefined) features[featureId].imageUrl = updates.imageUrl ? String(updates.imageUrl).trim() : null;
  if (updates.url !== undefined) features[featureId].url = updates.url ? String(updates.url).trim() : null;
  if (updates.subjectId !== undefined) features[featureId].subjectId = updates.subjectId ? String(updates.subjectId).trim() : null;
