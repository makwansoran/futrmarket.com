...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    // If we still have an error and no data, check if it's "Cannot coerce"
    if (error && (String(error.message || "").includes("Cannot coerce") || String(error.code || "") === "PGRST116")) {
      console.log("[updateContract] Cannot coerce error, but update succeeded. Returning updates applied.");
      // Update worked, return what we updated
      return {
        id: contractId,
        ...updatesToApply
      };
