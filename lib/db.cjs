...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    // After successful update, fetch the updated contract
    const { data, error } = await supabase
      .from("contracts")
      .select("*")
      .eq("id", contractId)
    ...
    // If we get "Cannot coerce" error on select, try a simpler select
    if (error && (String(error.message || "").includes("Cannot coerce") || String(error.code || "") === "PGRST116")) {
      console.log("[updateContract] Got Cannot coerce error on select, trying simpler select...");
      
      // Try selecting just the id to verify the contract exists
      ...
      // Contract exists, so the update likely worked. Try to get it with trending column explicitly
      const { data: simpleData, error: simpleError } = await supabase
        .from("contracts")
        .select("id, question, category, trending, live, featured, status, resolution, image_url, created_at, description, expiration_date, competition_id, subject_id, market_price, buy_volume, sell_volume, total_contracts, volume, yes_price, no_price, yes_shares, no_shares, created_by")
        .eq("id", contractId)
        .maybeSingle();
      
      if (!simpleError && simpleData) {
        console.log("[updateContract] Got contract with simple select, returning it");
        return simpleData;
      }
      
      // If that also fails, try getting the contract without the problematic select
      // The update succeeded, so we can construct a response from what we know
      console.warn("[updateContract] Select failed but update succeeded. Fetching contract with basic fields...");
      ...
      if (!basicError && basicData) {
        // Update worked, return a minimal response with the updates applied
        console.log("[updateContract] Update succeeded, returning minimal data with updates");
        return {
          ...basicData,
          ...updatesToApply
        };
    ...
    if (error) {
      console.error("[updateContract] Error in select after update:", error);
      console.error("[updateContract] Contract ID:", contractId);
      console.error("[updateContract] Updates:", JSON.stringify(updatesToApply, null, 2));
      console.error("[updateContract] Full error object:", JSON.stringify(error, null, 2));
      
      // If we already handled the "Cannot coerce" error above, we shouldn't get here
      // But if we do, the update worked but select failed - return what we can
      const errorMsg = String(error.message || "");
      const errorCode = String(error.code || "");
      const errorDetails = String(error.details || "");
      
      // If it's still a "Cannot coerce" error and we're updating trending, 
      // the update likely worked, so return the updates we applied
      if (trendingValue !== null && (errorMsg.includes("Cannot coerce") || errorCode === "PGRST116")) {
        console.log("[updateContract] Cannot coerce on select, but update succeeded. Returning updates applied.");
        // Get the contract with a very basic select to verify it exists
      ...
      // If we're trying to update trending and get "Cannot coerce", the column likely doesn't exist
      if (trendingValue !== null && (errorMsg.includes("Cannot coerce") || errorCode === "PGRST116")) {
        ...
        // Contract exists, so the error is definitely about the trending column not existing
        const clearError = new Error(
          ...
          `After running this SQL, try toggling trending again.`
        );
        clearError.code = "MISSING_COLUMN";
        clearError.details = "The trending column is missing from the contracts table";
        clearError.hint = "Run the ALTER TABLE command shown in the error message";
        throw clearError;
      }
      
      // If trending column doesn't exist (other error types)
      if (trendingValue !== null && (
