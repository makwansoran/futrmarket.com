...
async function createUser(userData) {
  ...
  // For wallet-first schema, we need a wallet_address as primary key
  // Generate a deterministic placeholder wallet address from email
  const crypto = require("crypto");
  const emailHash = crypto.createHash("sha256").update(email).digest("hex");
  const placeholderWallet = "0x" + emailHash.substring(0, 40); // 40 hex chars = 20 bytes
  
  const user = {
    wallet_address: placeholderWallet, // Required primary key for wallet-first schema
    email,
  ...
  if (isSupabaseEnabled()) {
    ...
    // Also create balance entry for the user
    try {
      await supabase
        .from("balances")
        .insert({
      ...
      console.error("Error creating balance (may already exist):", balanceError);
    }
