...
async function getAllFeatures(filterActive = false) {
  if (isSupabaseEnabled()) {
    ...
    // Note: We query for "Active" but also filter case-insensitively as a safety measure
    if (filterActive) {
    ...
    if (error) {
      console.error("❌ [getAllFeatures] Error fetching features:", error);
      console.error("❌ [getAllFeatures] Error code:", error.code);
      console.error("❌ [getAllFeatures] Error message:", error.message);
    ...
    if (features.length > 0) {
      console.log(`[getAllFeatures] Found ${features.length} total features:`, 
        features.map(f => ({ id: f.id, title: f.title, status: f.status })));
    }
    
    // Additional case-insensitive filtering as a safety measure
    // This handles cases where status might be "active", "ACTIVE", etc.
    if (filterActive) {
      ...
      if (beforeCount !== features.length) {
        console.warn(`[getAllFeatures] Filtered out ${beforeCount - features.length} features with non-matching status (case-insensitive check)`);
        console.warn(`[getAllFeatures] Features with other statuses:`, 
          data.filter(f => String(f.status || "").trim().toLowerCase() !== "active")
            .map(f => ({ id: f.id, title: f.title, status: f.status })));
      }
    }
    
    console.log(`[getAllFeatures] filterActive=${filterActive}, found ${features.length} features`);
    if (filterActive && features.length > 0) {
      ...
      console.warn("[getAllFeatures] No active features found! Check Supabase:");
      console.warn("   1. Make sure features exist in the database");
      console.warn("   2. Set status to exactly 'Active' (case-sensitive)");
      console.warn("   3. Check if RLS is blocking access (error code 42501)");
    }
