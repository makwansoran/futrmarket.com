...
async function createUser(userData) {
  ...
  try {
    const { error: balanceError } = await supabase
      ...
      if (balanceError) {
        console.error("⚠️  [createUser] Error creating balance (may already exist):", balanceError);
        // Don't throw - balance creation failure shouldn't prevent user creation
      } else {
        console.log("✅ [createUser] Balance created successfully for user:", walletAddress);
      }
  } catch (balanceError) {
...
async function getBalance(email) {
  ...
  if (isSupabaseEnabled()) {
    ...
    // Generate wallet_address from email (same as createUser does)
