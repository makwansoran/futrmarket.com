...
async function updateContract(contractId, updates) {
  if (isSupabaseEnabled()) {
    ...
    // If we get "Cannot coerce" error, it might mean the update affected 0 rows
    // Let's check if the contract exists and if the update actually worked
    if (error && (String(error.message || "").includes("Cannot coerce") || String(error.code || "") === "PGRST116")) {
      console.log("[updateContract] Got Cannot coerce error, checking if contract exists and update worked...");
      ...
      if (checkError) {
        console.error("[updateContract] Error checking contract:", checkError);
      }
      
      if (checkData) {
        console.log("[updateContract] Contract exists, checking if update worked...");
        // Contract exists, so the update might have worked but the select failed
        // Try to get the full contract to see if the update took effect
        ...
        if (!fullError && fullData) {
          console.log("[updateContract] Update appears to have worked, returning data");
          return fullData;
        }
      }
    }
